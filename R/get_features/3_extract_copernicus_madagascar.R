# get copernicus habitat data for sweden and madagascar ---------------------------------------
...
# boundaries for madagascar
madagascar       <- ne_countries(country = "Madagascar" , scale = "large")
...
madagascar_ftype <- raster::crop(x = ftype , y = madagascar) %>% mask( . , madagascar) 
...
for(i in seq_along(buffers)){
  ...
  # extract data & recode
  lc_ex <- raster::extract(madagascar_ftype ,  y = IBA_locs[,c("x","y")] , buffer = buffers[i] , df = TRUE) %>% as.data.frame()
...
# Get forest cover for madagascar
fcover        <- raster("data/raw_envdata/copernicus/PROBAV_LC100_global_v3.0.1_2018-conso_Tree-CoverFraction-layer_EPSG-4326.tif")
madagascar_fcover <- raster::crop(x = fcover , y = madagascar) %>% mask( . , madagascar) 

plot(madagascar_fcover)
...
for(i in seq_along(buffers)){
  ...
  # Extract data & recode
  lc_ex <- raster::extract(madagascar_fcover ,  y = IBA_locs[,c("x","y")] , buffer = buffers[i] , df = TRUE) %>% as.data.frame()
...
madagascar_crop_cover <- raster::crop(x = crop_cover , y = madagascar) %>% mask( . , madagascar) 
...
for(i in seq_along(buffers)){
  ...
  # Extract data & recode
  lc_ex <- raster::extract(madagascar_crop_cover ,  y = IBA_locs[,c("x","y")] , buffer = buffers[i] , df = TRUE) %>% as.data.frame()
...
madagascar_grass_cover <- raster::crop(x = grass_cover , y = madagascar) %>% mask( . , madagascar) 
...
for(i in seq_along(buffers)){
  ...
  # Extract data & recode
  lc_ex <- raster::extract(madagascar_grass_cover ,  y = IBA_locs[,c("x","y")] , buffer = buffers[i] , df = TRUE) %>% as.data.frame()
...
madagascar_shrub_cover <- raster::crop(x = shrub_cover , y = madagascar) %>% mask( . , madagascar) 

plot(madagascar_shrub_cover)
...
for(i in seq_along(buffers)){
  ...
  # Extract data & recode
  lc_ex <- raster::extract(madagascar_shrub_cover ,  y = IBA_locs[,c("x","y")] , buffer = buffers[i] , df = TRUE) %>% as.data.frame()
...
madagascar_water_cover <- raster::crop(x = water_cover , y = madagascar) %>% mask( . , madagascar) 

plot(madagascar_water_cover)
...
for(i in seq_along(buffers)){
  ...
  # Extract data & recode
  lc_ex <- raster::extract(madagascar_water_cover ,  y = IBA_locs[,c("x","y")] , buffer = buffers[i] , df = TRUE) %>% as.data.frame()
...
madagascar_water_s_cover <- raster::crop(x = water_s_cover , y = madagascar) %>% mask( . , madagascar) 

plot(madagascar_water_s_cover)
...
for(i in seq_along(buffers)){
  ...
  # Extract data & recode
  lc_ex <- raster::extract(madagascar_water_s_cover ,  y = IBA_locs[,c("x","y")] , buffer = buffers[i] , df = TRUE) %>% as.data.frame()
...
madagascar_urban_cover  <- raster::crop(x = urban_cover  , y = madagascar) %>% mask( . , madagascar) 

plot(madagascar_urban_cover)
...
for(i in seq_along(buffers)){
  ...
  # Extract data & recode
  lc_ex <- raster::extract(madagascar_urban_cover ,  y = IBA_locs[,c("x","y")] , buffer = buffers[i] , df = TRUE) %>% as.data.frame()
...
madagascar_snow_cover  <- raster::crop(x = snow_cover  , y = madagascar) %>% mask( . , madagascar) 

plot(madagascar_snow_cover)
...
for(i in seq_along(buffers)){
  ...
  # Extract data & recode
  lc_ex <- raster::extract(madagascar_snow_cover ,  y = IBA_locs[,c("x","y")] , buffer = buffers[i] , df = TRUE) %>% as.data.frame()
...
madagascar_moss_cover  <- raster::crop(x = moss_cover  , y = madagascar) %>% mask( . , madagascar) 

plot(madagascar_moss_cover)
...
for(i in seq_along(buffers)){
  ...
  # Extract data & recode
  lc_ex <- raster::extract(madagascar_moss_cover ,  y = IBA_locs[,c("x","y")] , buffer = buffers[i] , df = TRUE) %>% as.data.frame()
...
madagascar_discrete_class  <- raster::crop(x = discrete_class  , y = madagascar) %>% mask( . , madagascar) 

plot(madagascar_discrete_class)
...
for(i in seq_along(buffers)){
  ...
  # Extract data & recode
  lc_ex <- raster::extract(madagascar_discrete_class ,  y = IBA_locs[,c("x","y")] , buffer = buffers[i] , df = TRUE) %>% as.data.frame()
...
# save
saveRDS(saveList  , "tidy_data/fractional_cover/madagascar_copernicus_fractional_cover.rds")
...
# 
# par(mfrow=c(1,1))
# plot(madagascar_ftype, col = viridis::rocket(5 , end = 1 , direction = -1))
# 
# 
# DBF <- madagascar_ftype == 4 
# EBF <- madagascar_ftype == 2
# 
# 
# par(mfrow = c(1,2))
# plot(DBF, legend = FALSE , main="DBF")
# plot(EBF , legend = FALSE , main="EBF")
# 
# 
# fcover       <- raster("data/raw_envdata/copernicus/PROBAV_LC100_global_v3.0.1_2018-conso_Tree-CoverFraction-layer_EPSG-4326.tif")
# madagascar_fcover <- raster::crop(x = fcover , y = madagascar) %>% mask( . , madagascar) 
# 
# mfc_df <- madagascar_fcover %>% 
#           as(. , "SpatialPointsDataFrame") %>% 
#           data.frame() %>% 
#           dplyr::select(cover = 1 , x , y)
# 
# mft_df <- madagascar_ftype %>% 
#           as(. , "SpatialPointsDataFrame") %>% 
#           data.frame() %>% 
